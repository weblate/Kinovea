...
namespace Kinovea.ScreenManager
{
    public partial class PlayerScreenUserInterface : KinoveaControl, IDrawingHostView
    {
        ...
        /// <summary>
        /// Returns the speed factor over real time (scene time).
        /// </summary>
        public double SpeedFactorReal
        {
            get
            {
                return timeMapper.GetSpeedFactorReal(sldrSpeed.Value);
            }
            set
            {
                // This happens in the context of synching
                sldrSpeed.Update(timeMapper.GetInputFromSpeedFactorReal(value));
        ...
        /// <summary>
        /// Returns the speed factor with regards to the 
        /// original video file frame rate.
        /// Normally this should be very rarely used, as the nominal speed
        /// is considered arbitrary.
        /// </summary>
        public double SpeedFactorNominal
        {
            get { return timeMapper.GetSpeedFactorNominal(sldrSpeed.Value); }
        }
        ...
        // Speed mapping.
        // Time mapper links the speed slider, the nominal video file frame rate and
        ...
        // Current working zone and position.
        // Everything in media timestamps.
        // trkSelection.minimum and maximum are also in absolute timestamps.
        private VideoSection workingZone = VideoSection.MakeDefault();
        ...
        private void ClearKeyframeBoxes()
        {
            for (int i = keyframeBoxes.Count - 1; i >= 0; i--)
            {
                ...
                pnlThumbnails2.Controls.Remove(box);
        ...
        public int PostLoadProcess()
        {
            ...
            else if (currentTimestamp < 0)
            {
                ...
                m_FrameServer.Unload();
                log.Error(String.Format("First frame loaded but negative timestamp ({0}) - aborting", currentTimestamp));
                return -2;
            }
            ...
            m_FrameServer.Metadata.InitTime(workingZone.Start, workingZone.End, workingZone.Start);
            ...
            double speedFactorNominal = 1.0;
            if (screenDescriptor != null)
            {
                speedFactorNominal = screenDescriptor.SpeedFactorNominal;
            }
        ...
        {
            timeMapper.UpdateTimebase(
                m_FrameServer.VideoReader.Info.FrameIntervalMilliseconds,
                m_FrameServer.Metadata.BaselineFrameInterval,
                m_FrameServer.Metadata.HighSpeedFactor);
        }
        ...
        public void UpdateWorkingZone(CacheLoadMode loadMode)
        {
            ...
            {
                m_FrameServer.Metadata.TimeOrigin = workingZone.Start;
            }

            m_FrameServer.Metadata.InitTime(workingZone.Start, workingZone.End, m_FrameServer.Metadata.TimeOrigin);
            
            // Update Timeline and selection UI with resolved values.
            trkSelection.SelStart = workingZone.Start;
            trkSelection.SelEnd = workingZone.End;
            trkFrame.SetBounds(workingZone, m_FrameServer.VideoReader.Info.AverageTimeStampsPerFrame);
        ...
        private void ResetInternalData()
        {
            ...
            workingZone = VideoSection.MakeDefault();
        ...
        {
            trkSelection.Initialize(workingZone.Start, workingZone.End);
            UpdateSelectionLabels();
        }
        ...
        private void ChronometerStartStop()
        {
            foreach (var drawing in m_FrameServer.Metadata.ChronoManager.Drawings)
            {
                ...
                timeable.StartStop(currentTimestamp);
            }
        ...
        private void ChronometerSplit()
        {
            foreach (var drawing in m_FrameServer.Metadata.ChronoManager.Drawings)
            {
                ...
                timeable.Split(currentTimestamp);
            }
        ...
        private void CadenceBeat()
        {
            foreach (var drawing in m_FrameServer.Metadata.ChronoManager.Drawings)
            {
                ...
                timeable.Beat(currentTimestamp);
            }
        ...
        {
            if (!m_FrameServer.Loaded || m_FrameServer.Metadata.DrawingInitializing || isCurrentlyPlaying)
                return;
        ...
        public void Common_MouseWheel(object sender, MouseEventArgs e)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (((ModifierKeys & Keys.Shift) == Keys.Shift) && currentTimestamp <= workingZone.Start)
                        buttonGotoLast_Click(null, EventArgs.Empty);
                    else
                        buttonGotoPrevious_Click(null, EventArgs.Empty);
                }
        ...
        private void btn_HandlersLock_Click(object sender, EventArgs e)
        {
            ...
            if (m_FrameServer.Loaded)
            {
                ...
                trkSelection.SelLocked = selectionIsLocked;
        ...
        {
            if (!m_FrameServer.Loaded || selectionIsLocked)
                return;
        ...
        private void UpdateSelectionLabels()
        {
            ...
            if (m_FrameServer.Loaded)
            {
                start = workingZone.Start - firstTimestamp;
                duration = Math.Round(workingZone.End - workingZone.Start + m_FrameServer.VideoReader.Info.AverageTimeStampsPerFrame);
            }
            ...
            {
                duration = Math.Round(duration - m_FrameServer.VideoReader.Info.AverageTimeStampsPerFrame);
            }

            string durationTimecode = m_FrameServer.TimeStampsToTimecode((long)duration, TimeType.Duration, timecodeFormat, true);
            int right = lblSelDuration.Right;
            lblSelDuration.Text = "[" + durationTimecode + "]";
            lblSelDuration.Left = right - lblSelDuration.Width;
        }
        private void UpdateWorkingZoneDataFromControl()
        {
            if (workingZone.Start == trkSelection.SelStart && workingZone.End == trkSelection.SelEnd)
                return;

            // Time origin: we try to maintain user-defined time origin,
            // but we don't want the origin to stay at the video zero when the zone changes.
            // Check if we were previously aligned with the start of the zone, if so, keep it that way,
            // otherwise keep the absolute value.
            if (m_FrameServer.Metadata.TimeOrigin == workingZone.Start)
            {
                m_FrameServer.Metadata.TimeOrigin = trkSelection.SelStart;
            }

            workingZone = new VideoSection(trkSelection.SelStart, trkSelection.SelEnd);
        }

        private void AfterWorkingZoneChanged()
        {
            // Update everything as if we moved the handlers manually.
            m_FrameServer.Metadata.SelectionStart = workingZone.Start;
            m_FrameServer.Metadata.SelectionEnd = workingZone.End;
        ...
        private void trkFrame_PositionChanging(object sender, TimeEventArgs e)
        {
            ...
            {
                PresentFrame(trkFrame.Position);
            }
        ...
        private void MultimediaTimer_Tick(uint uTimerID, uint uMsg, UIntPtr dwUser, UIntPtr dw1, UIntPtr dw2)
        {
            ...
            lock (lockBusyRendering)
            {
                ...
                BeginInvoke((Action)delegate { PresentPlayback(); });
            }
        }

        /// <summary>
        /// Present the next frame to be rendered during playback based on the current time.
        /// </summary>
        private void PresentPlayback()
        {
            //------------------------------
            ...
            if (!isCurrentlyPlaying)
            {
                // Playback was stopped while the multimedia timer thread was asleep.
                return;
            }
        ...
        public void StopPlaying(bool present = true)
        {
            ...
            double elapsedTimestamps = GetPlaybackElapsedTimestamps();
            long expectedTimestamp = (long)Math.Round(activePlayerState.ReferenceTimestamp + elapsedTimestamps);
            expectedTimestamp = Math.Min(expectedTimestamp, workingZone.End);
        ...
        private void HandToolDown()
        {
            ...
            m_PointerTool.OnMouseDown(m_FrameServer.Metadata, transformer, m_iActiveKeyFrameIndex, m_DescaledMouse, currentTimestamp, defaultFadingEnabled);
        ...
        private void CreateNewDrawing(Guid managerId)
        {
            ...
            if (m_ActiveTool == ToolManager.Tools["Label"])
            {
                ...
                {
                    int hit = label.HitTest(m_DescaledMouse, currentTimestamp, distorter, transformer);
                    if (hit < 0)
                        continue;
            ...
            if (!editingLabel)
            {
                AbstractDrawing drawing = m_ActiveTool.GetNewDrawing(m_DescaledMouse, currentTimestamp, m_FrameServer.Metadata.AverageTimeStampsPerFrame, m_FrameServer.Metadata.ImageTransform);
        ...
        private void CreateNewMultiDrawingItem(AbstractMultiDrawing manager)
        {
            ...
            AbstractMultiDrawingItem item = manager.GetNewItem(m_DescaledMouse, currentTimestamp, m_FrameServer.Metadata.AverageTimeStampsPerFrame);
        ...
        private void SurfaceScreen_RightDown()
        {
            ...
            if (m_FrameServer.Metadata.IsOnDrawing(m_iActiveKeyFrameIndex, m_DescaledMouse, currentTimestamp))
            {
            ...
            else if ((hitDrawing = m_FrameServer.Metadata.IsOnDetachedDrawing(m_DescaledMouse, currentTimestamp)) != null)
            {
        ...
        private void PrepareDrawingContextMenu(AbstractDrawing drawing, ContextMenuStrip popMenu)
        {
            ...
            {
                bool gotoVisible = defaultFadingEnabled && (drawing.InfosFading.ReferenceTimestamp != currentTimestamp);
                if (gotoVisible)
                {
        ...
        private bool AddDrawingCustomMenus(AbstractDrawing drawing, ToolStripItemCollection menuItems)
        {
            ...
            if (drawing is DrawingChronoMulti)
                extraMenu = ((DrawingChronoMulti)drawing).GetContextMenu(currentTimestamp);
            else if (drawing is DrawingCounter)
                extraMenu = ((DrawingCounter)drawing).GetContextMenu(currentTimestamp);
            else
        ...
        private void PrepareFilterContextMenu(IVideoFilter filter, ContextMenuStrip popMenu)
        {
            ...
            List< ToolStripItem> menus = filter.GetContextMenu(m_DescaledMouse, currentTimestamp);
        ...
        private void SurfaceScreen_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            else if (e.Button == MouseButtons.Left)
            {
                if (m_ActiveTool != m_PointerTool)
                {
                    ...
                    else if (!isCurrentlyPlaying && m_iActiveKeyFrameIndex >= 0 && m_FrameServer.Metadata.HitDrawing != null)
                    {
                        IInitializable initializableDrawing = m_FrameServer.Metadata.HitDrawing as IInitializable;
                        if (initializableDrawing != null)
                            initializableDrawing.InitializeMove(m_DescaledMouse, ModifierKeys);
        ...
        private void SurfaceScreen_Paint(object sender, PaintEventArgs e)
        {
            ...
            timeWatcher.LogTime("Actual start of paint");
            ...
            timeWatcher.LogTime("Finished painting.");
        }
        ...
        private void FlushOnGraphics(Bitmap _sourceImage, Graphics g, Size _renderingSize, int _iKeyFrameIndex, long _iPosition, ImageTransform _transform)
        {
            ...
            // Drawings.
            if (
                (showDrawings && isCurrentlyPlaying && drawOnPlay) ||
                (showDrawings && !isCurrentlyPlaying))
            {
        ...
        {
            //-----------------------------------------------------------------    
            // We come here after loading the initial batch of keyframes but more likely
            // after an add, delete, or "manual sort" operation.
        ...
        public void GotoNextKeyframe()
        {
            ...
            {
                if (currentTimestamp < m_FrameServer.Metadata[i].Timestamp)
                {
                    next = i;
                    break;
                }
            ...
            if (next >= 0 && m_FrameServer.Metadata[next].Timestamp <= workingZone.End)
        ...
        public void GotoPreviousKeyframe()
        {
            ...
            if (prev >= 0 && m_FrameServer.Metadata[prev].Timestamp >= workingZone.Start)
        ...
        {
            int keyframeIndex = m_FrameServer.Metadata.GetKeyframeIndex(currentTimestamp);
            if (keyframeIndex >= 0)
            {
            ...
            if (KeyframeAdding != null)
                KeyframeAdding(this, new KeyframeAddEventArgs(currentTimestamp, null, Keyframe.DefaultColor));
        }
        ...
        public void AddPresetKeyframe(string name, Color color)
        {
            ...
            if (KeyframeAdding != null)
                KeyframeAdding(this, new KeyframeAddEventArgs(currentTimestamp, name, color));
        }
        ...
        private void InitializeKeyframe(Keyframe keyframe)
        {
            ...
            keyframe.InitializePosition(currentTimestamp);
        ...
        {
            m_iActiveKeyFrameIndex = m_FrameServer.Metadata.GetKeyframeIndex(currentTimestamp);
            OrganizeKeyframes();
            UpdateFramesMarkers();
            DoInvalidate();
        }
        public void UpdateKeyframes()
        {
            ...
            foreach (Keyframe kf in m_FrameServer.Metadata.Keyframes)
            {
                if (workingZone.Contains(kf.Timestamp))
                {
        ...
        private void KeyframeControl_MoveToCurrentTimeAsked(object sender, EventArgs e)
        {
            ...
            if (knownKeyframe == null)
            {
                ...
                newKf.Timestamp = currentTimestamp;
            }
            else
            {
                // Change the keyframe reference time.
                keyframe.Timestamp = currentTimestamp;
            }
        ...
        private void KeyframeControl_Selected(object sender, TimeEventArgs e)
        {
            ...
            if (!workingZone.Contains(e.Time))
                return;
        ...
        private void mnuDrawingTrackingTrimTracks_Click(object sender, EventArgs e)
        {
            ...
            m_FrameServer.Metadata.TrimAttachedTracks(drawing, currentTimestamp);
            UpdateFramesMarkers();
            RefreshImage();
        }
        ...
        private void DisplayClosestFrame(Point p, List<TimedPoint> trackPoints, float timeScale, bool use3D)
        {
            ...
            if (use3D)
            {
                ...
                for (int i = 0; i < trackPoints.Count; i++)
                {
                    ...
                    float dt = currentTimestamp - trackPoints[i].T;
                    dt /= timeScale;
        ...
        public void PaintFlushedImage(Bitmap output)
        {
            ...
            using (Graphics canvas = Graphics.FromImage(output))
            {
                ...
                FlushOnGraphics(m_FrameServer.CurrentImage, canvas, output.Size, keyframeIndex, currentTimestamp, m_FrameServer.ImageTransform.Identity);
            }
        }
